...
data_coin_v = data_coin_v.rename(columns={'GRP2': '경과일구분'})
...
data_kospi_v = data_kospi_v.rename(columns={'GRP2': '경과일구분'})
...
data_nasdaq_v = data_nasdaq_v.rename(columns={'GRP2': '경과일구분'})

data_nasdaq_a = data_nasdaq_a.rename(columns={'pred_day': '예측일'})


data_coin_v['경과일구분'] = data_coin_v['경과일구분'].replace({'A1_5' : 'Top5', 'B6_10' : 'Top6~10', 'C11_15' : 'Top11~15', 'D16_20' : 'Top16~20', 'Z21~all' : '기타' })
data_kospi_v['경과일구분'] = data_kospi_v['경과일구분'].replace({'A1_5' : 'Top5', 'B6_10' : 'Top6~10', 'C11_15' : 'Top11~15', 'D16_20' : 'Top16~20', 'Z21~all' : '기타' })
data_nasdaq_v['경과일구분'] = data_nasdaq_v['경과일구분'].replace({'A1_5' : 'Top5', 'B6_10' : 'Top6~10', 'C11_15' : 'Top11~15', 'D16_20' : 'Top16~20', 'Z21~all' : '기타' })
...
with col1 :
    st.write(data_coin_v[(data_coin_v['예측일'] == select_date) & (data_coin_v['랭킹룰']  == rule_rank) ][[
        '경과일구분','1D_고점상승비중','4D_고점상승비중','7D_고점상승비중','12D_고점상승비중']].T )
with col2 :
    st.write(data_coin_v[(data_coin_v['예측일'] == select_date) & (data_coin_v['랭킹룰']  == rule_rank) ][[
        '경과일구분', '1D_고점상승평균','4D_고점상승평균','7D_고점상승평균','12D_고점상승평균']].T )

st.markdown(f'###### 👈 1.2 예측 후 날짜경과별 <시가 대비 종가상승> 비중 및 평균값,  예측날짜: {select_date} 기준, 랭킹룰 : {rule_rank}')
col3,col4 = st.columns([1,1])
with col3 :
    st.write(data_coin_v[(data_coin_v['예측일'] == select_date) & (data_coin_v['랭킹룰']  == rule_rank) ][[
        '경과일구분','1D_종가상승비중','4D_종가상승비중','7D_종가상승비중','12D_종가상승비중']].T )
with col4 :
    st.write(data_coin_v[(data_coin_v['예측일'] == select_date) & (data_coin_v['랭킹룰']  == rule_rank) ][[
        '경과일구분','1D_종가상승평균','4D_종가상승평균','7D_종가상승평균','12D_종가상승평균']].T )
    
st.markdown(f'###### 👈 1.3 예측 후 날짜경과별 <시가 대비 저점상승> 비중 및 평균값,  예측날짜: {select_date} 기준, 랭킹룰 : {rule_rank}')
col5,col6 = st.columns([1,1])
with col5 :
    st.write(data_coin_v[(data_coin_v['예측일'] == select_date) & (data_coin_v['랭킹룰']  == rule_rank) ][[
        '경과일구분','1D_저점상승비중','4D_저점상승비중','7D_저점상승비중','12D_저점상승비중']].T )
with col6 :
    st.write(data_coin_v[(data_coin_v['예측일'] == select_date) & (data_coin_v['랭킹룰']  == rule_rank) ][[
        '경과일구분','1D_저점상승평균','4D_저점상승평균','7D_저점상승평균','12D_저점상승평균']].T )
